...
FDIOStream::FDIOStream(int _fd){
  ...
  int on = 1;
  if (ioctlsocket(fd, FIONBIO, (u_long*)&on) < 0)
  {
    closesocket(fd);
...
int FDIOStream::read(void* ptr, int numbytes){
	...
	while (totalRead < numbytes) {
		...
		if (FD_ISSET(fd, &fdSet)) {
		  numRead = recv(fd, bufferPointer, numbytes-totalRead, MSG_WAITALL); //greg1
...
int FDIOStream::write(const void* ptr, int numbytes){
	int numWrote = send(fd, (char*)ptr, numbytes, MSG_DONTROUTE); //greg1
	...
	if (numWrote == -1 && (errno == EWOULDBLOCK || errno == EAGAIN))
	{
		...
		if (FD_ISSET(fd, &fdSet)) {
			numWrote = send(fd, (char*)ptr, numbytes, MSG_DONTROUTE);//greg1
			if(numWrote==numbytes)
				return numWrote;
		}
	}
